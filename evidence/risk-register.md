...
| R-002 | Cross-case or cross-organization data access | PR.AA, PR.DS | Critical | Explicit authorization boundaries, server-side enforcement, and negative-path regression tests | Mitigated / monitored |
...
| R-004 | Provider or collected data creates false identity/location certainty | GV.RM, ID.RA, PR.DS | High | Recorded source authority, precision and uncertainty, and required human review before conclusions are drawn | Mitigating |
...
| R-006 | Critical provider, processing tier, or queue outage | GV.SC, PR.IR, RC.RP | High | Bounded retries, durable state, and failure visibility; recovery exercise required | Tracked |
...
| R-008 | Synthetic validation data contaminates real investigations or analytics | PR.DS, DE.AE | High | Structural separation of validation data from production data, and negative-path regression tests | Mitigated / monitored |
...
| R-010 | Backup, rollback, or incident recovery is not sufficiently proven | PR.IR, RS.MI, RC.RP | High | Controlled recovery, rollback, alerting, and representative service-failure exercises; recurring assurance and restricted internal evidence | Mitigating |
