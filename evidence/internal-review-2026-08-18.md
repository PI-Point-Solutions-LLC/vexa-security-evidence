...
The reconciliation boundary now deduplicates only exact transport replays. Altered reuse of an event identity is retained as a conflict, repeated conflict signals are idempotent, unknown objects cannot create canonical evidence, and byte-identical signals with a new transport identity remain auditable without rescanning already-cleared immutable bytes. A subject-free production drill exercised each outcome and left canonical trust unchanged. A separate immutable retention ledger now accepts idempotent, case-owner-authorized legal-hold and deletion requests; requests do not delete bytes or change trust, holds block deletion, and cross-case writes fail. Production authorization and hold enforcement were exercised in a rollback transaction that left no retention request or hold on the fixture. Physical deletion confirmation, separate-boundary restore, and cost gates remain tracked before cutover.
